...
import 'package:tradologie_app/features/dashboard/data/models/auction_unit_list_model.dart';
...
import 'package:tradologie_app/features/dashboard/data/models/get_vendor_stock_list_model.dart';
...
class DashboardRepositoryImpl implements DashboardRepository {
  ...
  Future<Either<Failure, List<GetVendorStockList>>> getReadyStockListing(
    ...
    try {
      final response =
          await dashboardRemoteDataSource.getReadyStockListing(params);
      if (response != null && response.success) {
        return Right((response.data as List)
            .map((e) => GetVendorStockListModel.fromJson(e))
            .toList());
      }
      return Left(UserFailure(response?.message, response?.code));
    } on Failure catch (e) {
  ...
  @override
  Future<Either<Failure, List<AuctionUnitList>>> getAuctionUnit(
      String params) async {
    try {
      final response = await dashboardRemoteDataSource.getAuctionUnit(params);
      if (response != null && response.success) {
        return Right((response.data as List)
            .map((e) => AuctionUnitListModel.fromJson(e))
            .toList());
      }
      return Left(UserFailure(response?.message, response?.code));
    } on Failure catch (e) {
